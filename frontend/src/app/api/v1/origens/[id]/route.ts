...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const row = await queryOne('SELECT * FROM origens WHERE id = $1 AND ativo = true', [params.id]);
  if (!row) return notFound('Origem não encontrada');
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const row = await queryOne(
      `UPDATE origens SET ${fields.join(', ')} WHERE id = $${idx} RETURNING *`,
      values,
    );
    return NextResponse.json(row);
  } catch (error: any) {
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const row = await queryOne(
    'UPDATE origens SET ativo = false, updated_at = NOW() WHERE id = $1 RETURNING *',
    [params.id],
  );
  return NextResponse.json(row);
}
